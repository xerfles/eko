...
# --- 🧠 CANLI HAFIZA SİSTEMİ (Session State) ---
...
# --- 💾 KAYIT VE ADİSYON SİSTEMİ ---
if st.button("💾 ANALİZİ KAYDET VE GELECEK ADİSYONUNU AL", use_container_width=True):
    # Excel Formatı (Virgüllü)
    def f_excel(val): return "{:.2f}".format(val).replace(".", ",")
    
    current_time = datetime.now().strftime("%d.%m.%Y %H:%M")
    
    # 1. Kanal: Excel'e (Sheets) Kayıt
    v_excel = [current_time, u_name, u_gender, f_excel(u_salary), u_prof, u_city, "0.0.0.0", f_excel(s_enf), f_excel(res_total), f_excel(tahmini_kur), f_excel(alim_kaybi), f_excel(1000/(1+res_total/100))]
    
    # 2. Kanal: Canlı Hafızaya (Sayısal) Kayıt (Admin Paneli İçin)
    v_live = {
        "Zaman Damgası": current_time, "Analist": u_name, "Cinsiyet": u_gender, "Maas": u_salary, 
        "Profil": u_prof, "Sehir": u_city, "IP": "0.0.0.0", "Tahmin": s_enf, "Yil_Sonu_Toplam": res_total,
        "Dolar_Beklentisi": tahmini_kur, "Alim_Gucu_Kaybi": alim_kaybi, "Reel_Kalan": 1000/(1+res_total/100)
    }
    st.session_state.live_data.append(v_live)
    
    if save_to_sheets(v_excel):
        st.balloons()
        st.markdown(f"""<div class="receipt-box"><center>🧾 <b>LiraPulse ADİSYON</b></center><hr><p><b>Analist:</b> {u_name}</p><p><b>Profil:</b> {u_prof}</p><p><b>Yıl Sonu Tahmini:</b> %{res_total:.2f}</p><p><b>1.000 TL Reel Değeri:</b> {round(1000/(1+res_total/100), 2):.2f} TL</p><hr><center><i>Veri Google Sheets'e Mermi Gibi İşlendi.</i></center></div>""", unsafe_allow_html=True)

# --- 🔐 ADMIN: CANLI HAFIZADAN BESLENİR ---
with st.expander("🔐 Admin Control Center"):
    if st.text_input("Şifre:", type="password", key="adm_pw") == "alper2026":
        try:
            # Önce Hafızadaki Veriyi Al, Boşsa Excel'e Bak
            if not st.session_state.live_data:
                client = get_gspread_client()
                sheet = client.open("LiraPulse_Veri").sheet1
                raw_df = pd.DataFrame(sheet.get_all_records())
                
                # Excel'den Çekerken Veriyi Temizle (Gümrük Memuru)
                def clean_num(val):
                ...
                if not raw_df.empty:
                    for _, row in raw_df.iterrows():
                        target_col = 'Yil_Sonu_Toplam' if 'Yil_Sonu_Toplam' in raw_df.columns else 'Yil_Sonu_Toplar'
                        st.session_state.live_data.append({
                            "Analist": row['Analist'], "Cinsiyet": str(row['Cinsiyet']).capitalize(),
                            "Maas": clean_num(row['Maas']), "Profil": row['Profil'], "Sehir": row['Sehir'],
                            "IP": row['IP'], "Yil_Sonu_Toplam": clean_num(row[target_col]),
                            "Dolar_Beklentisi": clean_num(row['Dolar_Beklentisi'])
                        })

            df_cloud = pd.DataFrame(st.session_state.live_data)
            ...
            if not df_cloud.empty:
                ...
                if st.button("🗑️ SEÇİLENLERİ SİL"):
                    # Hem hafızadan hem Excel'den sil
                    rows_to_keep = edited_df[edited_df["Seç"] == False].drop(columns=["Seç"])
                    st.session_state.live_data = rows_to_keep.to_dict('records')
