...
with col_out:
    ...
    with h3: st.metric("💰 1.000 TL Reel", f"{tr_format(reel_deger)} TL"); st.markdown(f'<p class="bugun-etiket">Yıl Sonu Değeri</p>', unsafe_allow_html=True)
...
if st.button("💾 ANALİZİ KAYDET VE ADİSYONU AL", use_container_width=True):
    ...
    if save_to_sheets(v):
        st.balloons()
        st.markdown(f"""<div class="receipt-box"><center>🧾 <b>LiraPulse ADİSYON</b></center><hr><p><b>Analist:</b> {u_name}</p><p><b>ID:</b> {kayit_id}</p><p><b>Enflasyon:</b> %{tr_format(res_total)}</p><p><b>Dolar Beklentisi:</b> {tr_format(tahmini_kur)} TL</p><p><b>1.000 TL Reel Değer:</b> {tr_format(reel_deger)} TL</p><hr><center><i>Veri Google Sheets'e Mermi Gibi İşlendi.</i></center></div>""", unsafe_allow_html=True)
...
with st.expander("🔐 Admin Control Center"):
    if st.text_input("Şifre:", type="password", key="adm_pw") == "alper2026":
        ...
        if len(st.session_state['admin_data']) > 0:
            ...
            # --- 🛡️ KRİTİK HATA ÖNLEYİCİ: GRAFİK ÇİZME ÖNCESİ KONTROL ---
            if not df.empty and len(df) > 0:
                g1, g2, g3 = st.columns(3)
                with g1: st.plotly_chart(px.pie(df, names='Cinsiyet', title="Cinsiyet", hole=0.4), use_container_width=True)
                with g2: st.plotly_chart(px.pie(df, names='Sehir', title="Şehir", hole=0.4), use_container_width=True)
