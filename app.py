...
st.set_page_config(page_title="LiraPulse Intelligence: Dashboard", layout="wide")
...
# --- 🛰️ HEADER ---
st.title("🛰️ LiraPulse Intelligence v21.0")

# --- 📊 1. ÜST DASHBOARD (YENİ) ---
...
with col_in:
    ...
    p_ps5 = st.number_input("PS5 (TL):", value=24000)
    p_iphone = st.number_input("iPhone 17 Pro (TL):", value=85000)
    p_car = st.number_input("Araba (TL):", value=1200000)
...
# --- 🏁 ANALİZ PANELİ ---
with col_out:
    # --- 📊 2. ANLIK ÖZET PANELİ (YENİ) ---
    st.markdown(f"""
    <div class="ozet-panel">
        <h4 style="margin:0; color:#888;">Anlık Beklenti Özeti</h4>
        <div style="display:flex; justify-content: space-around; margin-top:10px;">
            <div><small>Tahmini Kur</small><br><b style="font-size:24px; color:#00d4ff;">{tahmini_kur:.2f} TL</b></div>
            <div><small>Senin Enflasyonun</small><br><b style="font-size:24px; color:#ff4b4b;">%{res_total:.1f}</b></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.write("")
    ...
    with c_erime:
        ...
        st.caption(f"Yıl sonu alım gücü.")
...
# --- 🕰️ NOSTALJİ ---
st.subheader("🕰️ Zaman Makinesi: Asgari Ücret Gücü (2000-2026)")
df_nost = pd.DataFrame({"Yıl": ["2000", "2010", "2020", "BUGÜN", "2026 T."], "Dolar ($)": [115, 385, 380, 378, 17002 / tahmini_kur]})
st.plotly_chart(px.bar(df_nost, x="Yıl", y="Dolar ($)", text_auto='.0f', title="Maaş Kaç Dolar?", color="Dolar ($)", color_continuous_scale='Greens').update_layout(height=350), use_container_width=True)
...
# --- 💾 3. ANALİZİ KAYDET VE ADİSYON (SAYFA SONUNA GÖNDERİLDİ) ---
st.divider()
if st.button("💾 ANALİZİ KAYDET VE 2026 ADİSYONUNU AL", use_container_width=True):
    save_data(u_name, u_prof, u_city, res_total-Q1_ENF, res_total, tahmini_kur, "Genel", alim_kaybi, bin_tl_kalan)
    st.balloons()
