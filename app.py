...
def save_to_sheets(veri_listesi):
    try:
        ...
        # Google Sheets dosya adın mermi gibi aynı olmalı: LiraPulse_Veri
...
# --- 🎨 CSS TASARIMI ---
st.markdown("""<style>
    .stMetric { background-color: #161b22; padding: 15px; border-radius: 15px; border-left: 5px solid #00d4ff; }
    .ozet-panel { background: linear-gradient(145deg, #1e1e26, #252532); padding: 20px; border-radius: 15px; border: 1px solid #30363d; text-align: center; }
    .receipt-box { background-color: #fff; color: #333; padding: 20px; border-radius: 5px; font-family: 'Courier New', monospace; border: 2px dashed #333; margin-top: 20px; color: black !important; }
...
with col_in:
    ...
    u_salary = st.number_input("Maaş (TL):", value=22102)
...
with col_out:
    ...
    if st.button("💾 ANALİZİ KAYDET VE FİŞ AL", use_container_width=True):
        ...
        if save_to_sheets(veri):
            st.balloons()
            st.markdown(f"""<div class="receipt-box"><center>🧾 <b>LiraPulse ADİSYON</b></center><hr>Analist: {u_name}<br>Beklenen Enflasyon: %{res_total:.1f}<br>Dolar Tahmini: {tahmini_kur:.2f} TL<br><hr><center>Veri Buluta İşlendi.</center></div>""", unsafe_allow_html=True)
...
with st.expander("🔐 Yönetici Paneli"):
    if st.text_input("Şifre:", type="password") == "alper2026":
        try:
            ...
            st.write("Veri henüz yüklenemedi.")
