...
with col_out:
    ...
    st.divider()
    c1, c2 = st.columns(2)
    with c1: st.metric("💰 1.000 TL Alım Gücü", f"{tr_format(reel_deger)} TL"); st.markdown('<p class="bugun-etiket">Yıl Sonu Değeri</p>', unsafe_allow_html=True)
    with c2: st.plotly_chart(go.Figure(go.Indicator(mode="gauge+number", value=alim_kaybi, title={'text': "Alım Gücü Kaybı (%)"}, gauge={'bar': {'color': "#ff4b4b"}})).update_layout(height=180, margin=dict(l=20, r=20, t=30, b=20), paper_bgcolor="rgba(0,0,0,0)", font={'color': "white"}), use_container_width=True)
...
# --- 🔐 ADMIN PANELİ (FOTOĞRAF 3 PASTA GRAFİKLERİ) ---
            ...
            try:
                ...
                if len(vals) > 1:
                    ...
                    for i in range(1, len(vals)):
                        row = vals[i]
                        new_data.append({
                            "Tarih": row[0], "Analist": row[1], "Cinsiyet": row[2], "Maas": clean_num(row[3]),
                            "Profil": row[4], "Sehir": row[5], "Kayit_ID": str(row[6]), "Enflasyon": clean_num(row[8]),
