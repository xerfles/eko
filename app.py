...
# --- 📊 SOKAĞIN NABZI (YENİ İSTATİSTİK BÖLÜMÜ) ---
st.subheader("📊 Sokağın Nabzı: Toplum Ne Bekliyor?")
if os.path.exists(DB_FILE):
    try:
        df_stats = pd.read_csv(DB_FILE, on_bad_lines='skip')
        st_c1, st_c2, st_c3 = st.columns(3)
        st_c1.metric("Toplam Analiz", f"{len(df_stats)} Kişi")
        st_c2.metric("Ort. Enflasyon Beklentisi", f"%{pd.to_numeric(df_stats['Yil_Sonu_Toplam'], errors='coerce').mean():.1f}")
        st_c3.metric("En Çok Katılan Grup", df_stats['Profil'].mode()[0] if not df_stats['Profil'].empty else "-")
        
        g_c1, g_c2 = st.columns(2)
        with g_c1:
            st.plotly_chart(px.bar(df_stats['Profil'].value_counts().reset_index(), x='Profil', y='count', title="Kimler Daha Çok Beklenti Giriyor?", color='Profil'), use_container_width=True)
        with g_c2:
            st.plotly_chart(px.histogram(df_stats, x="Yil_Sonu_Toplam", nbins=20, title="Yıl Sonu Enflasyon Beklenti Dağılımı", color_discrete_sequence=['#ff4b4b']), use_container_width=True)
    except: st.info("İstatistikler toplanıyor...")
else: st.info("İlk analizi sen yap, istatistikler başlasın!")
...
with st.expander("🔐 Admin Control Center"):
    ...
    if admin_pass == "alper2026":
        if os.path.exists(DB_FILE):
            ...
            edited_df = st.data_editor(df_admin, column_config={"Seç": st.column_config.CheckboxColumn("Sil?", default=False)}, disabled=[c for c in df_admin.columns if c != "Seç"], use_container_width=True, key="admin_edit_v2")
            if st.button("🗑️ SEÇİLENLERİ SİL"):
                df_cleaned = edited_df[edited_df["Seç"] == False].drop(columns=["Seç"])
                df_cleaned.to_csv(DB_FILE, index=False); st.rerun()
